...
function upload_packages() {
    ...
    for p in \$(find \${pkg_path} -type f -name "*.rpm" ); do
        curl -s -u \${REPO_USER}:\${REPO_PASS} --upload-file \${p} \${repo_url}/\$(echo \${p} | awk -F '/' '{print \$NF}')
    done
}
...
upload_packages ./RPMS/x86_64 $REPO_BINARY
