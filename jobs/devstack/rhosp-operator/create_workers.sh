...
if [[ -n "$JUMPHOST" ]]; then
  ...
  if [[ "$PROVIDER" == 'bmc' ]]; then
    $WORKSPACE/src/tungstenfabric/tf-devstack/rhosp-operator/create_env.sh
...
src/tungstenfabric/tf-devstack/rhosp-operator/cleanup.sh
src/tungstenfabric/tf-devstack/rhosp-operator/create_env.sh
EOF
  ...
  for (( i=1; i<=$VM_BOOT_RETRIES ; ++i )) ; do
    ...
    if $WORKSPACE/src/tungstenfabric/tf-devstack/rhosp-operator/create_env.sh ; then
