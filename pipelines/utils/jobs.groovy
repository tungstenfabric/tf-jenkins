...
def _set_stream_jobs_code(def job_set, def stream, def lock) {
  def jobs_code = [:]
...
def _run_jobs(def job_set, def streams) {
  ...
  streams.keySet().each { name ->
    ...
    if (streams[name].containsKey('lock')) {
      jobs_by_stream[lock] = {
        lock(resource: streams[name]['lock']) { _set_stream_jobs(jobs, name) }
      }
    } else {
      jobs_by_stream[name] = _set_stream_jobs(jobs, name)
    }
  }

  // run jobs in parallel
  if (jobs_by_stream.size() > 0)
    parallel(jobs_by_stream)
}
